...
1. Cloud Shell 창에서 다음을 실행하여 이 랩에서 사용할 Windows 서버 2019를 실행하는 Azure VM을 배포합니다.
   ...
   ```
    ...
    > **참고**: 배포가 완료될 때까지 기다리지 말고 다음 작업을 진행하세요. 배포에는 최대 5분이 소요됩니다.
...
1. **az30310a-labRG - 액세스 제어(IAM)** 블레이드에서 **역할**을 클릭합니다.
...
1. 사용자 계정 목록에서 **az30310aaduser1** 사용자 계정을 표시하는 항목을 선택하고 도구 모음에서 줄임표 아이콘을 선택한 다음, **사용자 삭제**를 선택하고 확인하라는 메시지가 표시되면 **예**를 선택합니다.  
